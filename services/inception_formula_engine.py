...
for one instrument's full bar history, in a single forward pass — ported
verbatim from the backend's (now-removed) app/services/
inception_formula_engine.py, which this client used to call over the
network via /inception/snapshot and /inception/hmv. Those endpoints are
gone; this module is now the only place these values get computed, run
against bars synced locally by services.inception_sync_service/
inception_bars_store and orchestrated by services.inception_compute_service.

Three knobs that were fixed module constants on the backend are now
parameters (gap_threshold_pct/week_window_days/fifo_cap), each with the same
default the backend used, sourced from services.inception_settings — see
that module's docstring for why these three specifically are the ones
"editable formula" means for Group A/B (the algorithms themselves aren't
row-level expressions, so there's nothing else meaningful to expose).
...
Calendar rules: week = ISO week (Monday..Sunday, matching services/
formula_engine.py's own convention for LMV); quarter = calendar quarter
(Jan-Mar/Apr-Jun/Jul-Sep/Oct-Dec); half-year = Jan-Jun/Jul-Dec; financial
year = Apr 1 - Mar 31 (Indian FY, keyed by its starting calendar year).
...
min/max(previous-period-close, current-period-open). Stored as two metrics,
"<CODE> LOW"/"<CODE> HIGH", plus "<CODE> DATE" (when the gap opened) — see
inception_columns.gap_metric_names. "Closed back through" (what flips UF ->
FD) means a subsequent bar's close crosses back to the opposite side of the
PREVIOUS close (the pre-gap reference price): a gap-up area fills when close
<= that previous close; a gap-down area fills the mirror way (close >= it).

State per instrument/direction/period-type: two FIFOs (unfilled, filled),
each holding at most fifo_cap areas (default 3), most-recent-first (rank 1 =
index 0). A new day/week can (a) open a new gap (pushed onto the front of
`unfilled`, trimming to fifo_cap) if |pct change| exceeds gap_threshold_pct,
...
onto the front of `filled`, trimming to fifo_cap) — both can happen on the
same day for different pre-existing areas.
...
# column needs?" ─────────────────────────────────────────────────────────
#
# Uses the SAME period-key machinery the forward pass below uses (not a
...
def required_lookback_start(
    code: str, as_of_date: date, first_traded_date: date | None = None, week_window_days: int = DEFAULT_WEEK_WINDOW_DAYS,
) -> date | None:
    """The earliest `date_from` that makes *code* non-blank when HMV is
    evaluated as of *as_of_date* — see services.inception_compute_service's
    ...
    fixed_lookback_days = {
        ...
        "% CHG PWC AND OPEN": 14,
        "52WH": week_window_days, "52WL": week_window_days,
    }
    if code in fixed_lookback_days:
        return as_of_date - timedelta(days=fixed_lookback_days[code])
    if code in ("ATH", "ATL"):
...
def compute_group_a(bars: list[dict], week_window_days: int = DEFAULT_WEEK_WINDOW_DAYS) -> dict[date, dict]:
    """bars: ascending-by-trade_date rows (dicts with trade_date/open/high/
    low/close/volume/open_interest). Returns {trade_date: {code: value}} for
    every Group A code in inception_columns.GROUP_A.
    """
    if not bars:
        return {}
...
def _pct(base, current) -> float | None:
...
def _run_gap_state_machine(period_bars: list[dict], threshold_pct: float, fifo_cap: int) -> dict[date, dict]:
    """period_bars: ascending bars (daily, or the week-synthesized bars from
    _to_period_bars). Returns {period_end_date: {"UF_GUP": [...], "FD_GUP":
    [...], "UF_GDN": [...], "FD_GDN": [...]}} — each list holds up to
    fifo_cap _GapArea, most-recent-first — as of that bar (i.e. after that
    bar's close is applied).
    """
    ...
    for bar in period_bars:
        ...
        for direction, unfilled_key, filled_key, fills in (
            ...
            for area in fifos[unfilled_key]:
                if fills(area):
                    fifos[filled_key] = ([area] + fifos[filled_key])[:fifo_cap]
                else:
                    still_open.append(area)
            fifos[unfilled_key] = still_open[:fifo_cap]
        ...
        if prev_close is not None and prev_close != 0:
            ...
            if pct > threshold_pct:
                ...
                fifos["UF_GUP"] = ([area] + fifos["UF_GUP"])[:fifo_cap]
            elif pct < -threshold_pct:
                area = _GapArea(min(prev_close, bar["open"]), max(prev_close, bar["open"]), d, prev_close)
                fifos["UF_GDN"] = ([area] + fifos["UF_GDN"])[:fifo_cap]
...
def compute_group_b(
    bars: list[dict], threshold_pct: float = DEFAULT_GAP_THRESHOLD_PCT, fifo_cap: int = DEFAULT_FIFO_CAP,
) -> dict[date, dict]:
    ...
    change value on the first trading day of a new week — every day within a
    week sees the gap state as of the END of the previous (fully-completed)
    week, not the in-progress current week.
    """
    daily_state = _run_gap_state_machine(bars, threshold_pct, fifo_cap)
    week_bars = _to_period_bars(bars, "week")
    weekly_state_by_date = _run_gap_state_machine(week_bars, threshold_pct, fifo_cap)
